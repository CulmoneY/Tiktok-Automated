...
def movie_splitter(filename: str):
    ...
    for part in range(1, num_parts):
        # Make the part text
        txt_clip = (TextClip(f"Part {part}", fontsize=20, color='white', bg_color='red')
                    .set_position('center', 'top').set_duration(10))
    ...
    txt_clip = (TextClip(f"Final", fontsize=20, color='white', bg_color='red')
                .set_position('center', 'top').set_duration(final_duration)) # for subtitles, this should be a list of TextClips
    video_part = video.subclip(61 * (num_parts - 1))
    result = CompositeVideoClip([video_part, txt_clip])
    result.set_duration(video.duration - 61 * (num_parts - 1)).write_videofile(f"part_{num_parts}.mp4")
...
def subtitles(captions: list) -> list:
    ...
    for caption in captions:
        ...
        txt_clip = TextClip(text, fontsize=20, color='white')
        txt_clip.set_start(start_time).set_duration(end_time - start_time).set_position('center', 'bottom')
